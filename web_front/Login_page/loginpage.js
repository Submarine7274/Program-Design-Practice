...
function generateVerifyCode(){
    ...
    canvas.width=150;
    canvas.height=50;
    background.clearRect(0,0,canvas.width,canvas.height);   //清空畫布的動作
    const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";       //避免混淆字元
    for(let i =1;i<=5;i++){
        verifycode+=chars[Math.floor(Math.random()*chars.length)];  //random()是0~1的小數，乘以總長度，得到其中某個數如21.15245等，然後用floor去掉小數
    }
