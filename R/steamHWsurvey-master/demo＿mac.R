  ...
  walk(names(plot_list), ~ {
    ...
    if (!is.null(plots)) {
      ...
      if (!is.null(plots$trend)) {
        ggsave(filename = file.path(output_dir, sprintf("%s_trend.png", safe_name)), plot = plots$trend, width = 16, height = 9, dpi = 120)
      }
      if (!is.null(plots$change)) {
        ggsave(filename = file.path(output_dir, sprintf("%s_change.png", safe_name)), plot = plots$change, width = 16, height = 9, dpi = 120)
      }
    } else {
...
dynamic_font <- ifelse(current_os == "Darwin", "PingFang TC", "Microsoft JhengHei UI")
ui_config <- list(
  font_family = dynamic_font,
  base_size = 15, title_size = 22, subtitle_size = 14, axis_title_size = 16,
  axis_text_size = 13, legend_title_size = 14, legend_text_size = 13,
  label_size = 3.5, watermark_size = 4, nudge_ratio = 0.15,
  segment_color = "#555555", segment_linewidth = 0.8, segment_linetype = "dashed",
  color_normal = "#555555", color_alert = "#d73027", color_text = "#333333"
)
...
df_shs <- read_csv(target_file, show_col_types = FALSE) %>%
...
generate_category_plots <- function(dataset, target_category, top_n = 10) {
  ...
  plot_data <- cat_data %>% 
    ...
    mutate(
      clean_percentage = if (target_category == "Language") {
        case_when(date %in% as.Date(c("2020-12-01", "2023-03-01", "2024-05-01", "2026-02-01")) ~ NA_real_, TRUE ~ percentage)
      } else {
        case_when(
      ...
      smoothed_percentage = if (sum(!is.na(clean_percentage)) >= 2) na.approx(clean_percentage, na.rm = FALSE) else clean_percentage,
  ...
  p_trend <- plot_data %>%
    ...
    labs(title = paste("Steam 軟硬體歷史趨勢：", target_category), subtitle = "已修正抽樣異常與雜訊", x = "年份", y = "市佔率", color = "項目名稱") +
  ...
  if (target_category %in% c("Video Card Description", "VRAM")) {
    ...
    p_trend <- p_trend +
      ...
      annotate("text", x = as.Date("2021-09-15"), y = y_max * 0.95, label = shadow_label, color = ui_config$color_normal, fontface = "italic", size = ui_config$watermark_size, alpha = 0.6, family = ui_config$font_family)
  }
  ...
  if (!is.null(events)) {
    ...
    if (nrow(safe_events) > 0) {
      ...
      if (nrow(normal_events) > 0) p_trend <- p_trend + geom_vline(data = normal_events, aes(xintercept = date), color = ui_config$color_normal, linetype = "dashed", linewidth = 0.6, alpha = 0.8)
      if (nrow(alert_events) > 0) p_trend <- p_trend + geom_vline(data = alert_events, aes(xintercept = date), color = ui_config$color_alert, linetype = "dashed", linewidth = 0.7, alpha = 0.8)
      
      p_trend <- p_trend + geom_label_repel(
        data = safe_events, 
        aes(x = date, y = y_max, label = paste0(format(date, "%Y-%m-%d"), "\n", label)),
        nudge_y = current_nudge,              
        direction = "both", box.padding = 0.5, point.padding = 0.5, force = 5, max.overlaps = Inf, min.segment.length = 0, 
        hjust = 0.5, segment.color = ui_config$segment_color, segment.linetype = ui_config$segment_linetype, segment.size = ui_config$segment_linewidth, 
        fill = alpha("white", 0.8), color = ui_config$color_text, family = ui_config$font_family, size = ui_config$label_size, fontface = "bold", inherit.aes = FALSE      
      ) + coord_cartesian(clip = "off") + theme(plot.margin = margin(t = 90, r = 30, b = 10, l = 30))
    }
  }
  ...
  momentum_data <- plot_data %>%
    ...
    mutate(event_label = reorder(paste0(name, "\n(", format(date, "%Y-%m"), ")"), true_change))
  
  p_change <- ggplot(data = momentum_data, aes(x = true_change, y = event_label, fill = name)) +
    geom_col(width = 0.7) + 
    geom_text(aes(label = percent(true_change, accuracy = 0.1)), hjust = -0.2, family = ui_config$font_family, size = ui_config$label_size, fontface = "bold") +
    theme_minimal(base_size = ui_config$base_size, base_family = ui_config$font_family) +      
    theme(
    ...
    labs(title = paste("Steam 軟硬體歷史單月爆發力 Top 5：", target_category), subtitle = "各項目歷史最高單月增長率事件 (已排除抽樣異常)", x = "單月最高變化百分比 (Max MoM Change)", y = NULL) +
...
all_plots <- lapply(target_categories, function(cat) generate_category_plots(dataset = df_shs, target_category = cat))
