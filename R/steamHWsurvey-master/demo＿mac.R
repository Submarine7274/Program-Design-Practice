...
# ==========================================
# ----------------------------------------
# 資料讀取管線 (Data Input Pipeline)
# ----------------------------------------
if (requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::isAvailable()) {
  # 如果在 RStudio 環境下執行，自動取得本腳本路徑並設定為工作目錄
  current_script_path <- dirname(rstudioapi::getActiveDocumentContext()$path)
  setwd(current_script_path)
} else {
  # 若非 RStudio 環境 (例如在命令列)，則提醒使用者
  warning("未偵測到 RStudio 環境。請確保您的工作目錄 (getwd()) 已設定為腳本所在資料夾。")
}
# 1. 定義目標檔名 (位於當前工作目錄)
target_file <- "shs.csv"

# 2. 邊界檢查 (Bounds Checking)：確保檔案存在於當前目錄
